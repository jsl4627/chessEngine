...
public class Queen : Piece{ 
	...
	public override Dynarray<Coordinate> moves(Board board){ 
		...
		if(current_column < 8 && board.board[this.row, current_column].white != this.white){ 
			legal_moves.append(new Coordinate(this.row, current_column));
			potential_captures.append(board.board[this.row, current_column]); 
		...
		while(current_row < 8 && current_column < 8 && board.board[current_row, current_column] == null){ 
			legal_moves.append(new Coordinate(current_row, current_column)); 
			current_row += 1; 
			current_column += 1; 
		} 
		if(current_row < 8 && current_column < 8 && board.board[current_row, current_column].white != this.white){ 
			legal_moves.append(new Coordinate(current_row, current_column)); 
			this.potential_captures.append(board.board[current_row, current_column]); 
